...
$sql_code = "SELECT rmensal.cod_org AS cod_org, SUM(rmensal.result_mes) AS somaresultmes FROM indicadordepartamental AS inddep INNER JOIN indicadororgao AS indorg ON inddep.id_inddep = indorg.id_inddep INNER JOIN resultmensal AS rmensal ON rmensal.id_inddep = inddep.id_inddep WHERE inddep.id_inddep = '$recebevalor' AND indorg.cod_orgao = '$recebeorgao' GROUP BY 1 ";
