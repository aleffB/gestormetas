...
if($valid==1){
 ...
 $sql_code4 = "SELECT * FROM orgaos";
        ...
        $sql_code6 = "SELECT id.desc_inddep, id.melhor_traj, id.acumulativo, id.und_med,id.formula,io.peso, io.meta_fx1, io.meta_fx2
					FROM indicadororgao AS io 
					INNER JOIN indicadordepartamental AS id  ON io.id_inddep = id.id_inddep
					WHERE io.cod_orgao = '$orgaorec' AND id.anob = '$anorec'";
		$sql_query6 = $mysqli->query($sql_code6) or die($mysqli->error);
		$linhatabela6 = $sql_query6->fetch_assoc();
		//salvando em específio a melhor trajetória na variável $mjt
		$mjt = $linhatabela6['melhor_traj'];
        ...
        $sql_code6 = "SELECT id.desc_inddep, id.melhor_traj, id.acumulativo, id.und_med,id.formula,io.peso, io.meta_fx1, io.meta_fx2
					FROM indicadororgao AS io 
					INNER JOIN indicadordepartamental AS id  ON io.id_inddep = id.id_inddep
					WHERE io.cod_orgao = '$orgaobuscado' AND id.anob = '$anorec'";
		$sql_query6 = $mysqli->query($sql_code6) or die($mysqli->error);
		$linhatabela6 = $sql_query6->fetch_assoc();
		//salvando em específio a melhor trajetória na variável $mjt
		$mjt = $linhatabela6['melhor_traj'];
        ...
        //vetores para transformação de valores
		$melhortraj[1] = "Crescente";
  		$melhortraj[2] = "Na faixa";
  		$melhortraj[3] = "Decrescente";
  		$acum[1] = "Sim, acumulado no resultado";
  		$acum[2] = "Sim, acumulado na formula";
  		$acum[3] = "Não, o resultado é a média até o mês";
  		$undmed[1] = "R$";
  		$undmed[2] = "Unidade";
  		$undmed[3] = "%";
  		$undmed[4] = "Meses";
...
<input type="submit" name="confirmar" value="Confirmar" id="confirmar" class ="btn btn-success">	
...
<!-- criação de uma tabela -->
 <tr class= titulo>
   <td>INDICADOR</td>
   <td>Melhor</td> 
   <td>ACUMULATIVO(S OU N)</td> 
   <td>UNIDADE MEDIDA</td> 
   <td>FORMULA DO INDICADOR</td> 
   <td>PESO</td> 
  <!-- se ligar aqui --> <td>meta <?php ?></td> 
...
<?php
//verifica se a trejetória é 1 ou 3
if($mjt==1 || $mjt==3){
?>

<?php
//caso seja executará a tabela a baixo com a consulta do banco, deferenciando apenas o metafx que se for mjt igual a 1 ou 3 exibirá a meta fx1 apenas
do{

?>
 <tr class= titulo>
<!-- criação das linhas da tabela -->
 	<td id="descindep"><?php  echo $linhatabela6['desc_inddep'];?></td> 
   <td id="melhortraj"><?php echo $melhortraj[$linhatabela6['melhor_traj']];?></td> 
   <td id="acum"><?php echo $acum[$linhatabela6['acumulativo']];?></td> 
   <td id="undmed"><?php echo $undmed[$linhatabela6['und_med']];?></td> 
   <td id="form"><?php echo $linhatabela6['formula'];?></td> 
   <td id="peso"><?php echo $linhatabela6['peso'];?></td> 
   <td id="metafx"><?php echo $linhatabela6['meta_fx1'];?></td> 
 ...
 <!-- caso não seja mjt 1 ou 3 -->
 <?php }else{ ?>

<?php
// ele será na FAIXA ou seja mjt = 2, exibindo a consulta na tabela, diferenciando apenas o metafx, onde exibirá o meta fx1 e meta fx2
do{
...
?>
 <tr class= titulo>
   ...
   <td id="form"><?php echo $linhatabela6['formula'];?></td> 
   <td id="peso"><?php echo $linhatabela6['peso'];?></td> 
   <td id="metafx"><?php echo "Entre ".$linhatabela6['meta_fx1']." e ".$linhatabela6['meta_fx2'] ;?></td> 
