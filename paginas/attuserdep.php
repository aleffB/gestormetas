...
if($valid==2){
  ...
  switch (get_post_action('confirmar','deletar','buscar')) {

    case 'buscar':

   $buscarcpf = $_POST['cpfb'];

   foreach ($_POST as $chave=>$valor) 
    $_SESSION[$chave] = $mysqli->real_escape_string($valor);

  $sql_code = "SELECT * FROM administrador WHERE matricula ='$buscarcpf'";
     ...
     $lcpf = $linhas['cpf_adm'];
     $lrg = $linhas['rg_adm'];
     $lnasc = $linhas['datanasc_adm'];
     $lsexo = $linhas['sexo_adm'];
  ...
  $rrcpf = $_POST['recebecpf'];
	...
	foreach ($_POST as $chave=>$valor) 
    $_SESSION[$chave] = $mysqli->real_escape_string($valor);
		$sql_code = "DELETE FROM administrador WHERE matricula ='$rrcpf'";
...
<center>
<div id="attuserdep" style="width: 400px;">
<h1>Alterar Usuários/Excluir Usuários Dependentes</h1>
...
<form action="" method="POST">
   <p class=espaco></p>
    <label for="cpfb"> Insira a matrícula do dependente: </label>
    <input name="cpfb" required value="" class="form form-control" style="text-align: center;">
     <input value="Buscar" name="buscar" type="submit" class ="btn btn-success">
    <p class=espaco></p> 
</form>
<p class=espaco></p>
<form action="" method="POST">
    ...
    <input name="nome" required value="<?php echo $linhas['nome_adm']; ?>" maxlength="30" class="form form-control" style="text-align: center;">
    <p class=espaco></p>

    <label for="nomeuser"> Nome User: </label>
    <input name="nomeuser" required value="<?php echo $linhas['nomeuser_adm']; ?>" maxlength="30" class="form form-control" style="text-align: center;">
    <p class=espaco></p>
  

      <!----  SEM REQUIRED validar com JS -->
    <label for="telefone">Telefone:</label>
    <input name="telefone" value="<?php echo $linhas['telefone_adm']; ?>" required class="form form-control" style="text-align: center;">
    <p class=espaco></p>

   

    <label for="emailadm"> E-mail:</label>
    <input name="emailadm" required value="<?php echo $linhas['email_adm']; ?>" type="email" placeholder="Ex:nome@email.com" maxlength="100" class="form form-control" style="text-align: center;">
    <p class=espaco></p>
        

    <label for="datafinal"> Expirar Conta:</label>
    <input name="datafinal" required value="<?php echo $linhas['datafinal_adm']; ?>" type="date" readonly="true" id="dataf" class="form form-control" style="text-align: center;">
    <p class=espaco></p>
    
    <input name="recebe" id="recebe" required value="<?php echo $valid; ?>" type="numeric" hidden="true" >
    <input name="recebecpf" required value="<?php echo $buscarcpf; ?>"  hidden="true" >
    <p class=espaco></p>

    <input value="Atualizar" name="confirmar" type="submit" class ="btn btn-success" >
    <p class=espaco></p>
    <input value="Deletar" name="deletar" type="submit" class ="btn btn-danger" >
    <p class=espaco></p>
