 ...
 $sql_code4 = "SELECT * FROM orgaos";
    ...
    $_SESSION[$chave] = $mysqli->real_escape_string($valor);
            ...
            if( $resgateagain == $codorg  && $resgate == $iddep ){
              $recidmeta = $_POST['idmeta'];
              $sql_code= "UPDATE meta SET
            desc_meta = '$_SESSION[descmeta]',
            responsavel = '$_SESSION[response]'
            WHERE id_meta = '$recidmeta'";

          $confirma = $mysqli->query($sql_code) or die($mysqli->error);

          $correct = "Meta cadastrada com sucesso.";
          $acc++;
        }else{
...
if(isset($_POST['delete'])){
  ...
  foreach ($_POST as $chave=>$valor) 
    $_SESSION[$chave] = $mysqli->real_escape_string($valor);
    $sql_code = "DELETE FROM meta WHERE id_meta = '$recidmeta'";
...
<form action="" method="POST">
 <?php 
  ...
  <select name="descorgao" required id="descorgao" class="form form-control" style="text-align: center;">
   ...
   do{ 
  ?>
  <option value="<?php  echo $linhatabela['cod_orgao']; ?>"> <?php echo $linhatabela['desc_orgao'];?></option>
 ...
 <option value="<?php  echo $linhatabela2['anob']; ?>"> <?php echo $linhatabela2['anob'];?></option>
  ...
  <input name="descmeta" required value="" type="text" class="form form-control" style="text-align: center;" id="descmeta">
  <p class=espaco></p>
  ...
  <input type="numeric" name="idmeta" id="idmeta" hidden="true">
...
$('#anobase').on('mouseout', function(){
  ...
  $.post('paginas/reqattmeta.php', {valorSelect: valor, recebeOrgao: recebeorgao}, function(data) {
    $('#descmeta').html('');
    $('#response').html('');
    $('#idmeta').html('');
    ...
    $.each($.parseJSON(data), function(){
     document.getElementById("descmeta").value = this.desc_meta;
     document.getElementById("response").value = this.responsavel;
     document.getElementById("idmeta").value = this.id_meta;
